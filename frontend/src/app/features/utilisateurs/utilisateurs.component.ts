...
import { UtilisateurService, ReferenceService } from '../../core/services/reference.service';
...
@Component({
        ...
        <table mat-table [dataSource]="utilisateurs()" class="full-width">
          ...
          <ng-container matColumnDef="role">
            ...
            <td mat-cell *matCellDef="let u">
              <mat-chip [color]="getRoleColor(u.role)" highlighted>{{ u.role }}</mat-chip>
            </td>
          </ng-container>
          ...
          <ng-container matColumnDef="actif">
            ...
            <td mat-cell *matCellDef="let u">
              <mat-slide-toggle [checked]="u.actif" (change)="toggleActif(u)"
      ...
      <div class="dialog-overlay" (click)="closeDialog()">
        <mat-card class="dialog-card" (click)="$event.stopPropagation()">
          ...
          <mat-card-actions align="end">
            ...
            <button mat-raised-button color="primary" (click)="saveUser()" [disabled]="saving()">
  ...
  styles: [`
    ...
    .dialog-card { width: 560px; max-width: 95vw; border-radius: 16px !important; }
...
export class UtilisateursComponent implements OnInit {
  ...
  openDialog(): void { this.userForm.reset({ role: 'VENDEUR' }); this.showDialog.set(true); }
  ...
  saveUser(): void {
    ...
    this.utilisateurService.create(this.userForm.value).subscribe({
      next: () => {
        this.snackBar.open('Utilisateur créé', '✓', { duration: 3000, panelClass: ['snack-success'] });
        this.closeDialog(); this.loadUsers(); this.saving.set(false);
      },
  ...
  toggleActif(user: Utilisateur): void {
    this.utilisateurService.toggleActif(user.id).subscribe({
      next: () => {
        this.snackBar.open(`Utilisateur ${user.actif ? 'désactivé' : 'activé'}`, '✓', { duration: 2000 });
        this.loadUsers();
      }
    });
  }
  ...
  getRoleColor(role: string): string {
    return { ADMIN: 'warn', GESTIONNAIRE: 'primary', VENDEUR: 'accent' }[role] ?? '';
  }
}
